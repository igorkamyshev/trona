...
getFiles(evolutionsDir)
    ...
    .then((files) => Promise.all(files.map((file) =>
        getFileContent(path.join(evolutionsDir, file))
            .then((data) => ({
                data,
                file,
                checksum: md5(data),
    ...
    .then(files =>
        runQuery(`SELECT id, checksum, down_script FROM ${tableName} ORDER BY id ASC;`)
            .then((evolutions) =>
                ({files, evolutions})
            )
    )
    .then(({files, evolutions}) => {
        ...
        return invalidEvolution
            .reduceRight((promise, {id, down_script}) => {
                return promise
                    .then(() => {
                        console.log('')
                    ...
                    .then(() => runQuery(`DELETE FROM ${tableName} WHERE id = ${id};`))
            }, Promise.resolve())
