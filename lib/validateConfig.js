...
module.exports = function validateConfig (config) {
    let {tableName = 'evolutions', runQuery, evolutionsFolderPath = ['evolutions']} = config
    ...
    if (!evolutionsFolderPath.reduce((carry, pathChunk) => carry && typeof pathChunk === 'string', true)) {
        console.error(Red, 'Invalid config: evolutionsFolderPath should be a string or Array<string>')
        process.exit(1)
    }
